...
export function ContainerTypeModal({
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!containerType)
      return api
        .post("/container-types", { name })
        .then(() => {
          setIsLoading(false);
          Swal.fire(
            "Sucesso!",
            "Tipo de container criado com sucesso!",
            "success"
          );
          closeModal();
        })
    ...
    api
      .put(`/container-types/${containerType?.id}`, {
        ...containerType,
        name,
      })
      .then(() => {
        ...
        Swal.fire(
          "Sucesso!",
          "Tipo de container editado com sucesso!",
          "success"
        );
        closeModal();
      })
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {containerType?.id
              ? "Editar tipo de container"
              : "Criar tipo de container"}
          </h3>
          <button
