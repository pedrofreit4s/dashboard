...
interface IIncotermsModalProps {
  closeModal(): void;
  incoterm?: IIncoterm;
}
interface IForm {
...
export function IncotermModal({ closeModal, incoterm }: IIncotermsModalProps) {
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!incoterm)
      return api
        ...
        .then(() => {
          setIsLoading(false);
          Swal.fire("Sucesso!", "Incoterm criado com sucesso!", "success");
          closeModal();
        })
        .catch((err) => {
          setIsLoading(false);
    ...
    api
      .put(`/incoterms/${incoterm?.id}`, {
        ...incoterm,
        name,
      })
      .then(() => {
        setIsLoading(false);
        Swal.fire("Sucesso!", "Incoterm editado com sucesso!", "success");
        closeModal();
      })
      .catch((err) => {
        setIsLoading(false);
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {incoterm?.id ? "Editar incoterm" : "Criar incoterm"}
          </h3>
          <button
