...
export function CoinModal({ closeModal, coin }: UserModalProps) {
  const [isLoading, setIsLoading] = useState(false);

  const { handleSubmit, register } = useForm<IForm>({
    defaultValues: {
      ...coin,
      full_name: coin?.full_name === null ? "" : coin?.full_name,
    },
  });

  const onSubmit = useCallback(({ name, full_name }: IForm) => {
    setIsLoading(true);

    if (!coin)
      return api
        .post("coins", { name, full_name })
        .then(() => {
          setIsLoading(false);
          Swal.fire("Sucesso!", "Moeda criada com sucesso!", "success");
          closeModal();
        })
        .catch((err) => {
          setIsLoading(false);
    ...
    api
      .put(`coins/${coin?.id}`, { ...coin, name, full_name })
      .then(() => {
        setIsLoading(false);
        Swal.fire("Sucesso!", "Moeda editada com sucesso!", "success");
        closeModal();
      })
      .catch((err) => {
        setIsLoading(false);
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {coin?.id ? "Editar moeda" : "Criar moeda"}
          </h3>
          <button
