...
export function ModalModal({ closeModal, modal }: IUFModalProps) {
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!modal)
      return api
        ...
        .then(() => {
          setIsLoading(false);
          Swal.fire("Sucesso!", "Modal criado com sucesso!", "success");
          closeModal();
        })
        .catch((err) => {
          setIsLoading(false);
    ...
    api
      ...
      .then(() => {
        setIsLoading(false);
        Swal.fire("Sucesso!", "Modal editado com sucesso!", "success");
        closeModal();
      })
      .catch((err) => {
        setIsLoading(false);
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {modal?.id ? "Editar modal" : "Criar modal"}
          </h3>
          <button
