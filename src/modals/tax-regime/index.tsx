...
export function TaxRegimeModal({ closeModal, taxRegime }: UserModalProps) {
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!taxRegime)
      return api
        .post("tax-regimes", { name })
        .then(() => {
          setIsLoading(false);
          Swal.fire("Sucesso!", "Regime criado com sucesso!", "success");
          closeModal();
        })
        .catch((err) => {
          setIsLoading(false);
    ...
    api
      .put(`tax-regimes/${taxRegime?.id}`, { ...taxRegime, name })
      .then(() => {
        setIsLoading(false);
        Swal.fire("Sucesso!", "Regime editado com sucesso!", "success");
        closeModal();
      })
      .catch((err) => {
        setIsLoading(false);
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {taxRegime?.id ? "Editar regime" : "Criar regime"}
          </h3>
          <button
