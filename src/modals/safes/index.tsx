...
interface ISafeModalProps {
  closeModal(): void;
  safe?: ITypeOfEstimate;
}
interface IForm {
  name: string;
}

export function SafeModal({ closeModal, safe }: ISafeModalProps) {
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!safe)
      return api
        ...
        .then(() => {
          setIsLoading(false);
          Swal.fire("Sucesso!", "Seguro criado com sucesso!", "success");
          closeModal();
        })
        .catch((err) => {
          setIsLoading(false);
    ...
    api
      ...
      .then(() => {
        setIsLoading(false);
        Swal.fire("Sucesso!", "Seguro editado com sucesso!", "success");
        closeModal();
      })
      .catch((err) => {
        setIsLoading(false);
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {safe?.id ? "Editar seguro" : "Criar seguro"}
          </h3>
          <button
