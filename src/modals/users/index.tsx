...
export function UserModal({ closeModal, user }: UserModalProps) {
  const [isLoading, setIsLoading] = useState(false);

  const { handleSubmit, register } = useForm<IForm>({
    defaultValues: { ...user, password: undefined },
  });

  const onSubmit = useCallback(
    ({ name, email, password, confirm_password }: IForm) => {
      setIsLoading(true);
      if (password !== confirm_password)
        return Swal.fire("Erro!", "As senhas não são iguais!", "error");
      ...
      api
        .put(`accounts/${user?.id}`, { ...user, name, email, password })
        .then(() => {
          setIsLoading(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="mt-4">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                autoComplete="off"
                {...register("confirm_password", { required: !user?.id })}
              />
            </div>
          </div>
