...
interface ITypeOfEstimateModalProps {
  closeModal(): void;
  typeOfEstimate?: ITypeOfEstimate;
}
interface IForm {
  name: string;
}

export function TypeOfEstimateModal({
  closeModal,
  typeOfEstimate,
}: ITypeOfEstimateModalProps) {
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!typeOfEstimate)
      return api
        .post("/type-of-estimate", { name })
        .then(() => {
          setIsLoading(false);
          Swal.fire(
            "Sucesso!",
            "Tipo de estimativa criada com sucesso!",
            "success"
          );
          closeModal();
        })
    ...
    api
      .put(`/type-of-estimate/${typeOfEstimate?.id}`, {
        ...typeOfEstimate,
        name,
      })
      .then(() => {
        ...
        Swal.fire(
          "Sucesso!",
          "Tipo de estimativa editada com sucesso!",
          "success"
        );
        closeModal();
      })
