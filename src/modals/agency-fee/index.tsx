...
export function AgencyFeeModal({ closeModal, agencyFee }: IUFModalProps) {
  ...
  const onSubmit = useCallback(({ name }: IForm) => {
    ...
    if (!agencyFee)
      return api
        .post("/agency-fees", { name })
        .then(() => {
          setIsLoading(false);
          Swal.fire(
            "Sucesso!",
            "Taxa de agenciamento criada com sucesso!",
            "success"
          );
          closeModal();
        })
    ...
    api
      ...
      .then(() => {
        ...
        Swal.fire(
          "Sucesso!",
          "Taxa de agenciamento editada com sucesso!",
          "success"
        );
        closeModal();
      })
      ...
      >
        <div className="p-8 pb-2 flex items-center justify-between">
          <h3 className="font-work text-lg font-bold">
            {agencyFee?.id
              ? "Editar taxa de agenciamento"
              : "Criar taxa de agenciamento"}
          </h3>
          <button
